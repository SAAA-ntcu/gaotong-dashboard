...
<template>
  <div class="subject360-module">
    <div class="subject360-page-head">
      <div><h2>資料總覽</h2><p>{{ scopeLabel }}｜依序查看班級、能力向度、題目與學生資料。</p></div>
      <span class="subject360-badge">查看路徑</span>
    </div>
    ...
    <div class="subject360-kpi-grid">
      ...
      <div class="subject360-kpi"><span>優先查看題目</span><strong>{{ highItems }}</strong><small>符合多項條件</small></div>
    </div>
    ...
    <div class="subject360-decision-card">
      ...
      <small>這是建議查看順序，不是固定能力判定；單一訊號仍需回到題目與學生資料確認。</small>
    </div>

    <div class="subject360-grid subject360-grid-3">
      <article class="subject360-card subject360-focus-card"><span>第一步：查看班級</span><strong>{{ topClass ? `${topClass.classId} 班` : '—' }}</strong><p>{{ topClass ? formatPercent(topClass.stat.rate) + ' 整體答對率' : '目前沒有可排序的班級訊號' }}</p><button type="button" @click="emit('open-page', 'classes')">查看班級與能力 →</button></article>
      <article class="subject360-card subject360-focus-card"><span>第二步：查看能力</span><strong>{{ topDimension?.dimension?.key || '—' }}</strong><p>{{ topDimension ? formatPoints(topDimension.gap) + ' vs ' + comparisonLabel : '目前沒有可用向度' }}</p><button type="button" @click="topDimension && emit('open-ability', topDimension)">查看能力診斷 →</button></article>
      <article class="subject360-card subject360-focus-card"><span>第三步：題目 → 學生</span><strong>Q{{ topItem?.item.q || '—' }} {{ topItem?.item.short || '' }}</strong><p>{{ topItem ? formatPercent(topItem.selected.rate) + ' 所選範圍答對率' : '目前沒有可用題目' }}</p><button type="button" @click="topItem && emit('open-item', topItem.item.q)">查看題目，再查看學生 →</button></article>
    </div>

    <div class="subject360-grid subject360-grid-2">
      <article class="subject360-card">
        <div class="subject360-card-head"><h3>優先處理項目</h3><span>依序查看資料</span></div>
        ...
        <p class="subject360-caption">這是建議查看順序，不是固定能力判定；進入試題資料後，可點擊受影響學生開啟抽屜。</p>
      </article>
      <article class="subject360-card">
        <div class="subject360-card-head"><h3>重點摘要</h3><span>目前可先查看的訊號</span></div>
