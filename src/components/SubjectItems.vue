...
<template>
  <div class="subject360-module">
    <div class="subject360-page-head"><div><h2>試題資料</h2><p>查看選項分布與班級差異，再回到題目確認教學問題。</p></div><label class="subject360-inline-control">排序<select v-model="itemSort"><option value="priority">教學優先度</option><option value="q">題號</option><option value="rate">答對率低到高</option><option value="classGap">班級落差低到高</option></select></label></div>
    <div class="subject360-filter-bar"><label>{{ hasCognitive ? '內容 × 認知' : '內容向度' }}<select v-model="linkedFilter"><option v-for="key in linkedKeys" :key="key" :value="key">{{ key === 'all' ? '全部' : key }}</option></select></label><label>優先度<select v-model="priorityFilter"><option value="all">全部</option><option value="高優先">優先查看</option><option value="中優先">需查看</option><option value="建議觀察">觀察</option></select></label></div>
    ...
    <div class="subject360-grid subject360-grid-2">
      ...
      <article class="subject360-card"><div class="subject360-card-head"><h3>解讀提示</h3><span>題目 → 選項</span></div><div class="subject360-signal-list"><div><b>分母</b><p>答對率使用有效選項作答數 N；選項圖使用全部學生列。</p></div><div><b>主要錯誤</b><p>主要錯誤選項表示錯答集中，不等同迷思概念已被確診。</p></div><div><b>查看</b><p>點選題目可查看選項分布、班級比較與需要回到題目確認的學生。</p></div></div></article>
    </div>
