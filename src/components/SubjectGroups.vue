...
const guidance = {
  '特定概念待確認': {
    ...
    action: '先做錯誤選項比較，再回到代表題拆解解題步驟。'
  },
  '作答表現不穩定／需進一步診斷': {
    criteria: '整體表現、缺答比例或不同向度之間出現明顯落差。',
    action: '先補做短題組或口頭診斷，確認是概念、閱讀或作答狀態問題。'
  },
  '多題共同待支持': {
    criteria: '兩題以上共同偏低，且弱點向度有重疊學生。',
    action: '安排小組概念釐清，完成後用原題或平行題確認是否改善。'
  }
};
</script>
...
<template>
  <div class="subject360-module">
    <div class="subject360-page-head"><div><h2>教學分組</h2><p>依本次作答證據形成暫時性教學需求群組，不把學生固定貼上高、中、低分標籤。</p></div><span class="subject360-badge">{{ scopeLabel }}</span></div>
    ...
    <div class="subject360-group-grid">
      <article v-for="group in groups" :key="group.name" class="subject360-group-card">
        ...
        <div class="subject360-group-body">
          ...
          <div class="subject360-group-guidance"><div><b>形成條件</b><p>{{ guidance[group.name]?.criteria }}</p></div><div><b>建議行動</b><p>{{ guidance[group.name]?.action }}</p></div></div>
